...
if [ -d "$database_directory" ]; then
    ...
    python ../update_forward_test_files.py --db_name="$database_directory/database.xml" -z 1 -g "UnitBaseCorrelatedGrid" -v 2 --refine_electron_secondary_grids
    if [ $? -eq 0 ]; then
    ...
    python ../update_forward_test_files.py --db_name="$database_directory/database.xml" -z 1 -g "CorrelatedGrid" -v 3 --refine_electron_secondary_grids
    if [ $? -eq 0 ]; then
