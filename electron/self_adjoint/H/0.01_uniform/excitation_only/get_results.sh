...
# Get excitation only self-adjoint results
cd ./results
echo -e "\nGet excitation only self-adjoint results:"
  # Copy results to this location
  scp -r aci2:${INSTALL}/self_adjoint/H/0.01_uniform/excitation_only/results/* ./

  # Erase files from cluster
  if [ "$delete" = "true" ]; then
    ssh aci2 "rm -rf ${INSTALL}/self_adjoint/H/0.01_uniform/excitation_only/results/*"
  fi
cd ../