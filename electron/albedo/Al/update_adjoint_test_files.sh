...
if [ "${scatter_above_max_mode}" = "on" ]; then
  python ../../update_adjoint_test_files.py -d ${database} -z 13000 -g ${grid_policy} -i "${ionization}" -v ${version}
else
  python ../../update_adjoint_test_files.py -d ${database} -z 13000 -g ${grid_policy} -i "${ionization}" -v ${version} --scatter_above_max_mode_off
fi
