...
do
  ...
  printf "#!/bin/bash\n${python_command}" > update_H_${version}_temp.sh
  ${sbatch_command} update_H_${version}_temp.sh
  if [ ! $? -eq 0 ]; then
      printf "\nH native data version ${version} FAILED to update!\n"
      rm update_H_${version}_temp.sh
      exit 1
  fi
  rm update_H_${version}_temp.sh
done
