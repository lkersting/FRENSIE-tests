...
do
  ...
  do
    ...
    if [ "${nudge_mode}" = "on" ]; then
      ...
      python_command="python ../../update_adjoint_test_files.py -d ${DATABASE_PATH} -z 1000 -e 0.01 -g ${grid_policy} -v ${version} ${convergence_tol} ${eval_tol}"
      ...
      python_command="python ../../update_adjoint_test_files.py -d ${DATABASE_PATH} -z 1000 -e 0.01 -g ${grid_policy} -v ${version} ${convergence_tol} ${eval_tol} --scatter_above_max_mode_off"
      printf "#!/bin/bash\n${python_command}" > update_H_adjoint_temp.sh
      ${sbatch_command} update_H_adjoint_temp.sh
      rm update_H_adjoint_temp.sh
    fi
