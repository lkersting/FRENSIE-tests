...
if [ "$#" -eq 1 ]; then
  ...
  mpiexec -n ${SLURM_NTASKS} python -c "import example; example.runSimulationFromRendezvous(${SLURM_CPUS_PER_TASK}, ${HISTORIES}, ${TIME}, \"${RENDEZVOUS}\" )"
...
# Run new simulation
  ...
  python_script="example_${SLURM_JOB_ID}"
  cp example.py ${python_script}.py
